            ...
            if class_type == "unset" and proc.get("cache_type") == "Cache":
                ...
                for cur_role in proc.get("roles", None):
                    if cur_role["role"] == "log":
                        ...
                        log_mem_queue = cur_role["input_bytes"]["counter"] - cur_role["durable_bytes"]["counter"]
                        # print(cur_role["input_bytes"]["counter"], log_mem_queue)
                if log_queue_used is not None and log_mem_queue is not None:     
                    records.append({
                        "timestamp": ts,
                        "role": "Log_queue",
                        "cpu_usage": log_mem_queue,
...
def plot_data(dfs, group_interval, filenames, outfile,
              sys_df_sqlite, exp_window_sqlite):
    """
    绘图，共 6 个子图：
      - 左上区：
          子图1：操作日志 —— sec vs. ops_sec_sum（比较 cache 与 sqlite 的吞吐量）
          子图2：操作日志 —— sec vs. latency_avg（比较 cache 与 sqlite 的延迟）
      - 右上区：
          子图3：系统监控 —— CPU 使用率（绘制 4 条曲线：Cache/SQLite, server1/server2）
          子图4：系统监控 —— Disk 使用率（绘制 4 条曲线）
      - 左下区（新加）：
          子图5、子图6：示例（请根据需要修改绘图逻辑）
    ...
    # sys_cache_log_queue = sys_df_cache[sys_df_cache['role'] == "Log_queue"]
    sys_sqlite_log_queue = sys_df_sqlite[sys_df_sqlite['role'] == "Log_queue"]
    
    # 创建整个画布，并用 GridSpec 划分区域
    fig = plt.figure(figsize=(16, 16))
    # 将整个画布分为 2 行 2 列：
    # 上半部（outer_gs[0, :]）用来放置原来的 4 个图
    # 下半部左边（outer_gs[1, 0]）用来放置新加的 2 个图；右下角（outer_gs[1, 1]）留空
    outer_gs = fig.add_gridspec(2, 2, height_ratios=[2, 1], hspace=0.3, wspace=0.3)
    
    # ─────────────
    # 上半部区域
    # ─────────────
    # 左上区：原来第 1、2 张图（操作日志相关），在 outer_gs[0, 0] 内再划分 2 行 1 列
    gs_tl = outer_gs[0, 0].subgridspec(2, 1, hspace=0.3)
    ax1 = fig.add_subplot(gs_tl[0, 0])  # 子图1：操作日志吞吐量
    ax2 = fig.add_subplot(gs_tl[1, 0])  # 子图2：操作日志延迟

    # 右上区：原来第 3、4 张图（系统监控），在 outer_gs[0, 1] 内再划分 2 行 1 列
    gs_tr = outer_gs[0, 1].subgridspec(2, 1, hspace=0.3)
    ax3 = fig.add_subplot(gs_tr[0, 0])  # 子图3：CPU 使用率
    ax4 = fig.add_subplot(gs_tr[1, 0])  # 子图4：Disk 使用率
    
    # ─────────────
    # 下半部区域
    # ─────────────
    # 左下区：新加的 2 张图，在 outer_gs[1, 0] 内再划分 2 行 1 列
    gs_bl = outer_gs[1, 0].subgridspec(2, 1, hspace=0.3)
    ax5 = fig.add_subplot(gs_bl[0, 0])  # 子图5：新加图1（示例）
    ax6 = fig.add_subplot(gs_bl[1, 0])  # 子图6：新加图2（示例）
    
    # 注意：outer_gs[1, 1] 未使用，如果不需要可以不创建该区域
    ...
    # 子图5 & 子图6：新加图（下面仅给出占位示例，你可根据需要替换为具体绘图逻辑）
    # 示例：假设绘制某种“资源利用率”的数据（你可以传入新的 DataFrame 或使用已有数据）
    # 这里仅以空白图示例
    # print(sys_cache_log_queue)
    ax5.set_title("Log Server Memory Queue Size")
    ax5.set_ylabel("Memory Queue (byte)")
    # ax5.plot(sys_cache_log_queue['sec'], sys_cache_log_queue['cpu_usage'], label="Cache Log Memory")
    ax5.plot(sys_sqlite_log_queue['sec'], sys_sqlite_log_queue['cpu_usage'], label="Sqlite Log Memory")
    ax5.legend()
    ax5.grid(True)
    # 例如：ax5.plot(new_df['sec'], new_df['resource_metric'], label="Resource Metric")
    # ax5.legend()

    ax6.set_title("Log Server Disk Queue Size")
    ax6.set_ylabel("Disk Queue Size (byte)")
    ax6.set_xlabel("Seconds from Experiment Start")
    # ax6.plot(sys_cache_log_queue['sec'], sys_cache_log_queue['disk_usage'], label="Cache Disk Queue Size")
    ax6.plot(sys_sqlite_log_queue['sec'], sys_sqlite_log_queue['disk_usage'], label="Sqlite Disk Queue Size")
    ax6.legend()
    ax6.grid(True)
    # 例如：ax6.plot(new_df['sec'], new_df['other_metric'], label="Other Metric")
    # ax6.legend()
...
def main(group_interval, datasize, operation, data_dir="/mydata"):
    ...
    exp_window_sqlite = exp_windows[0]
